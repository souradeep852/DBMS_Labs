...
Node *BpTree::getParent(Node *node, int a)
{
    ...
    else if (a < node->data.at(0))
        return getApprDataNode(node->ptrs.at(0), a);
    else if (a > node->data.at(2 * t))
        return getApprDataNode(node->ptrs.at(2 * t + 1), a);
    else
    {
    ...
    else
    {
        ...
        else
        {
            ...
            if (parent->getFreeSLots() > 0)
            {
                ...
                loop(i, 2 * t + 1)
                {
                    if (parent->data.at(i) == INT_MAX)
                    {
                        upto = i;
                        break;
                    }
                }
                for (int i = upto; i > idx; i--)
                    parent->data.at(i) = parent->data.at(i - 1);
                parent->data.at(idx) = split->data.at(0);
                parent->decrFreeSLots();
                for (int i = upto + 1; i > idx + 1; i--)
                    parent->ptrs.at(i) = parent->ptrs.at(i - 1);
                parent->ptrs.at(idx + 1) = split;
                return;
            }
            else
